...
async def cases_create_template(
    ...
    if required_fields:
        params["requiredFields"] = [{"id": f} for f in required_fields]
...
async def cases_list_cases_for_contact(
    ...
    client = get_cases_client()
    return client.list_cases_for_contact(
        domainId=domain_id,
...
# Case Domains
async def cases_create_domain(name: str, description: str = "") -> dict:
    """Create a case domain."""
    client = get_cases_client()
    return client.create_domain(name=name, description=description)


async def cases_list_domains(max_results: int = 10) -> dict:
    """List case domains."""
    client = get_cases_client()
    return client.list_domains(maxResults=min(max_results, 10))


async def cases_get_domain(domain_id: str) -> dict:
